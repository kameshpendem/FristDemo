...
// import parseDate from '../../../../screens/HomeScreen/ParseDate';
...
class ReminderAlarms extends Component {
  constructor(props) {
    super(props);
    global.getdate = Date();
    this.state = {
      ...
      item.timedetails.map((i1) => {
        ...
        if (moment(dat).toDate() >= moment().toDate()) {
          ...
          let alarmNotifData = {
            ...
            title: global.hlpid + '@' + getBaseUrl(), // Required
  ...
  callMedicineUpdate = (notification_id, mytime, status) => {
    ...
    if (Time2 <= moment(new Date()).format('YYYY-MM-DD')) {
      if (Time1 <= LastTenMin) {
        if (status != '') {
          Alert.alert(
            ' Did you want to change medicine taken status again? ',
            null,
            [
              {
                text: 'Confirm',
  ...
  getAlert = (notification_id, Time1, LastTenMin) => {
    Alert.alert('Did you want to change medicine taken status', null, [
      {
        text: 'Taken',
        onPress: () =>
  ...
  render() {
    ...
    return (
      <ScrollView
          ...
          this.state.nightlist.length == 0 && (
            <View style={{justifyContent: 'center', alignItems: 'center'}}>
              <Text
                ...
                {i18n.t('CONTACTUS.NO_MEDICINE')}
              </Text>
            </View>
          )}
        {this.state.morninglist.length > 0 && (
          <Card title={i18n.t('CONTACTUS.MRNG')}>
        ...
        {this.state.afternoonlist.length > 0 && (
          <Card title={i18n.t('CONTACTUS.AF_NOON')}>
        ...
        {this.state.eveninglist.length > 0 && (
          <Card title={i18n.t('CONTACTUS.EVENG')}>
        ...
        {this.state.nightlist.length > 0 && (
          <Card title={i18n.t('CONTACTUS.NIGHT')}>
